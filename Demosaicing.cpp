...
void ConvertToThreeChannelBayerBG(Mat &BGRImage){
	...
	Mat mSrc_Bayer(BGRImage.rows, BGRImage.cols, CV_8UC1);
	int channel;
...
void Demosaicing(std::string &BayerFileName, cv::Mat &Dst, int rows, int cols, int BayerPatternFlag, int DemosaicingMethod){
    ...
    RawImage.convertTo(mSrc_Bayer, CV_32FC1);
	mSrc_Bayer.convertTo(mSrc_Bayer, CV_8UC1, 255.0 / 1023.0); // divide depend on img bit, care opencv saturate_case
    ...
    switch(BayerPatternFlag){ //turn all to RGGB by copyMakeBorder
        ...
        case 2: //GRBG
            // R|GR|G
            // G|BG|B
            copyMakeBorder(mSrc_Bayer, mSrc_Bayer, 0, 0, 1, 1, cv::BORDER_REFLECT_101);
            break;
        case 3: //BGGR
            // R|GR|G
            // ------
            ...
            // ------
            // G|BG|B
            copyMakeBorder(mSrc_Bayer, mSrc_Bayer, 1, 1, 1, 1, cv::BORDER_REFLECT_101);
            break;
        case 4: //GBRG
            // RG
            // --
            ...
            // --
            // GB
            copyMakeBorder(mSrc_Bayer, mSrc_Bayer, 1, 1, 0, 0, cv::BORDER_REFLECT_101);
            break;
        default:
    ...
    switch(DemosaicingMethod){
        case 1:
            demosaic_smooth_hue(mSrc_Bayer, Dst);
            break;
        case 2:
            demosaic_laplacian_corrected(mSrc_Bayer, Dst);
            break;
        case 3:
            demosaic_GBTF(mSrc_Bayer, Dst);
            break;
        case 4:
            demosaic_residual(mSrc_Bayer, Dst);
            break;
        default:
    ...
    switch(BayerPatternFlag){ //turn all back from RGGB to Dst
        ...
        case 2: //GRBG
            // R|GR|G
            // G|BG|B
            Dst = Dst(Rect(1, 0, mSrc_Bayer.cols - 2, mSrc_Bayer.rows));
            break;
        case 3: //BGGR
            // R|GR|G
            // ------
            ...
            // ------
            // G|BG|B
            Dst = Dst(Rect(1, 1, mSrc_Bayer.cols - 2, mSrc_Bayer.rows - 2));
            break;
        case 4: //GBRG
            // RG
            // --
            ...
            // --
            // GB
            Dst = Dst(Rect(0, 1, mSrc_Bayer.cols, mSrc_Bayer.rows - 2));
            break;
    }
...
void Demosaicing(cv::Mat mSrc_Bayer, cv::Mat& mDst_RGB, int rows, int cols, int BayerPatternFlag, int DemosaicingMethod) {
	...
	switch (DemosaicingMethod) {
	case 1:
		//demosaic_smooth_hue(mSrc_Bayer, mDst_RGB);
		demosaicing(mSrc_Bayer, mDst_RGB, cv::COLOR_BayerBGGR2RGB_EA);
		break;
	case 2:
		//demosaic_laplacian_corrected(mSrc_Bayer, mDst_RGB);
		demosaicing(mSrc_Bayer, mDst_RGB, cv::COLOR_BayerBGGR2RGB_VNG);
		break;
	case 3:
		demosaic_GBTF(mSrc_Bayer, mDst_RGB);
		break;
	case 4:
		demosaic_residual(mSrc_Bayer, mDst_RGB);
		break;
	default:
		std::cerr << "Wrong Demosaicing Method Index" << endl;
		return;
	...
	//Rect (x, y, width, height);
	switch (BayerPatternFlag) { //turn all back from RGGB to mDst_RGB
	case 1: //RGGB
		break;
	case 2: //GRBG
		// R|GR|G
		// G|BG|B
		mDst_RGB = mDst_RGB(Rect(1, 0, mSrc_Bayer.cols - 2, mSrc_Bayer.rows));
		break;
	case 3: //BGGR
		// R|GR|G
		// ------
		...
		// ------
		// G|BG|B
		mDst_RGB = mDst_RGB(Rect(1, 1, mSrc_Bayer.cols - 2, mSrc_Bayer.rows - 2));
		break;
	case 4: //GBRG
		// RG
		// --
		...
		// --
		// GB
		mDst_RGB = mDst_RGB(Rect(0, 1, mSrc_Bayer.cols, mSrc_Bayer.rows - 2));
		break;
	}
