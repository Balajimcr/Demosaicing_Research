...
void to_SingleChannel(cv::Mat &src,cv::Mat &dst)
{
    ...
    for(int i=0;i<src.rows;++i)
    {
        ...
        {
            int baseJ = j * 3;
            int sum = sptr[baseJ] + sptr[baseJ + 1] + sptr[baseJ + 2];
            dptr[j] = cv::saturate_cast<uchar>(sum);
        }
    }
}
    ...
    for (i = 5, k = 0; i < HGradientMap.rows - 5; ++i)
    {
        ...
        for (; j < HGradientMap.cols - 5; j+=2)
        {
            ...
            for(int dir=0;dir<4;++dir,++k)
            {
                ...
                if(WeightTable[k]) { 
                    //WeightTable[k] = 1.0 / (WeightTable[k]*WeightTable[k]); 
                    WeightTable[k] = 1.0 / (WeightTable[k] * WeightTable[k] + 1e-10);
                }
            }
        }
    }
...
void GBTF_CFAInterpolation(cv::Mat &Bayer,cv::Mat &Dst,int BayerPatternFlag = 0)
{
    ...
    // Prb.csv reading
    cv::Mat Prb(7, 7, CV_32FC1, cv::Scalar(0));
    PrbMat(Prb);
    ...
    for(i=0;i<height;++i)
    {
        ...
        for(;j<width;j+=2)
        {
            ...
            DstPtr[j * channels + 2] = cv::saturate_cast<uchar>(std::round(G - GRdiff / 4.0));
            DstPtr[j * channels] = cv::saturate_cast<uchar>(std::round(G - GBdiff / 4.0));
        }
    }
